...
import type { Dimension, LayoutTree, NodeCache, NodeOptions, RootAdjustment, ReactiveValue } from '../type';
...
export class Node {
    static cache: NodeCache = {
        ...
        nodOpts: createReactiveState(new Map<string, NodeOptions>(), areNodeOptionsMapEqual),
        bndOpts: createReactiveState(new Map<string, NodeOptions>(), areNodeOptionsMapEqual),
    }
    ...
    calcDimensions() {
        ...
        let totalWeight = 0;
        let totlMinSize = Node.cache.dimMins.get(this.unId) || { minWidth: 0, minHeight: 0 };
        let curntOffset = 0;
        ...
        for (const kid of this.kids) {
            ...
            const kidDims = Node.cache.dimMins.get(kid.unId) || { minWidth: 0, minHeight: 0 };
...
export class Layout {
    ...
    constructor(
        ...
        Layout._root = new Node('row', null, 'root', 100, { w: 0, h: 0, x: 0, y: 0 }, "root");
        this.nodeOps = createReactiveState(new Map<string, NodeOptions>(), areNodeOptionsMapEqual)
        ...
        console.log(`Node layout creation took ${endTime - startTime}ms`);
    ...
    *NodeLayoutRecursiveIterator(root: Node): IterableIterator<Node> {
    ...
    updateChildDirections(parent: Node) {
        ...
        if (parentDir === undefined) {
            console.warn(`Cannot update child directions for node ${parent.unId}; its own direction is not set.`);
            return;
        }
    ...
    createNodeTreeFromJSONTree(json: LayoutTree, host: Node) {
        ...
        if (host.type === 'tabset')
            Node.cache.tabsets.set(host.unId, host)
        if (host.type === 'tab')
            Node.cache.mapTabs.set(host.unId, host)
     ...
     * @param host 
     */
    createBinaryNodeTreeFromQueue(queue: Queue<string>, host: Node = Layout._root) {
        ...
        console.log(JSON.stringify(Layout._root.toJSON(), null, 2));
    ...
    calcTabsetCountAndMinDim(root: Node = Layout._root, clear: boolean = true) {
        ...
        for (const node of this.NodeLayoutRecursiveIterator(root)) {
            ...
            const hostCnt = tabCnts
                .set(node.unId, { horizontal: 0, vertical: 0 })
                .get(node.unId);

            const hostDim = dimMins
                .set(node.unId, { minWidth: 0, minHeight: 0 })
                .get(node.unId)
            ...
            if (node.type === 'tab') {
                ...
                dimMins.set(node.unId, { minWidth: 0, minHeight: 0 });
            }
            ...
            if (node.type === 'tabset') {
                ...
                dimMins.set(node.unId, { minWidth: Layout._minW, minHeight: Layout._minH });

                if (nodeDir) {
                    tabCnts.set(node.unId, { horizontal: 0, vertical: 1 });
                } else {
                    tabCnts.set(node.unId, { horizontal: 1, vertical: 0 });
                }
            }
            ...
            if (node.type === 'row') {
                for (const child of node.kids) {
                    ...
                    if (nodeDir) {
                        hostDim!.minHeight = Math.max(hostDim!.minHeight, kidDim!.minHeight!)
                        hostDim!.minWidth += kidDim!.minWidth!
                    } else {
                        hostDim!.minHeight += kidDim!.minHeight!
                        hostDim!.minWidth = Math.max(hostDim!.minWidth, kidDim!.minWidth!)
                    }
                }
    ...
    updateDimension(dim: Dimension, disableTimeout: boolean = false, timeout: number = 2) {
    ...
    updateSlider(id: string, dim: { x: number, y: number }, disableTimeout: boolean = false, timeout: number = 2) {
    ...
    updateSliderDimension(id: string, dim: { x: number, y: number }) {
        ...
        const prevMinDim = Node.cache.dimMins.get(prev.unId) || { minWidth: 0, minHeight: 0 };
        const nextMinDim = Node.cache.dimMins.get(next.unId) || { minWidth: 0, minHeight: 0 };
    ...
    updateTree(src: string, des: string, layout: 'top' | 'bottom' | 'left' | 'right' | 'contain'): boolean {
        let srcNode = Node.cache.mapNode.get(src);
        let desNode = Node.cache.mapNode.get(des);

        if (srcNode?.type == 'tab' && srcNode.host === desNode && desNode.kids.size() < 2) {
        ...
        if (srcNode === desNode) {
            console.warn('Source and destination nodes are the same, no action taken.');
            return false;
        }

        if (!srcNode.host || (!desNode.host && desNode.type !== 'row')) {
            console.warn(`Source node or destination node's host is null: ${srcNode.unId}, ${desNode.unId}`);
            return false;
        }

        if ((desNode.type === 'row') && layout === 'contain') {
            console.warn(`Cannot move to a row with 'contain' layout: ${desNode.unId}`);
            return false;
        }
        ...
        if (desNode.type === 'tab' && (layout === 'contain' || layout === 'left' || layout === 'right')) {
    ...
    moveRelativeToRoot(src: Node, des: Node, layout: 'top' | 'bottom' | 'left' | 'right') {
        this.removeNode(src);
    ...
    moveRelativeToTabset(src: Node, des: Node, layout: 'top' | 'bottom' | 'left' | 'right') {
    ...
    insertNode(des: Node, src: Node, flag: boolean) {
        ...
        if (desIdx === -1) {
            console.warn(`Destination node ${des.unId} not found in its host's kids.`);
            return;
        }

        let hostDir = Node.cache.mapDirs.get(desHost.unId);
    ...
    insertNodeWithNewParent(src: Node, des: Node, flag: boolean) {
        ...
        if (des === Layout._root) {
            ...
            if (des.kids.size() == 1 && des.kids.peek()?.type === 'tabset') {
                ...
                let rowA = new Node('row', des);
                let rowB = new Node('row', rowA);
    ...
    moveAdjacentNodeToGrandParent(src: Node) {
        ...
        if ((src.type == 'row') && srcHost) {
            ...
            for (const kid of src.kids) {
                ...
                Node.cache.mapDirs.set(kid.unId, !Node.cache.mapDirs.get(gndHost.unId)!);
    ...
    removeNode(src: Node) {
        ...
        if (
            ...
            kid && this.moveAdjacentNodeToGrandParent(kid);
...
export class Bond {
    ...
    toJSON(): any {
        return {
            uidBond: this.unId,
            next: this.next ? this.next.unId : null,
            prev: this.prev ? this.prev.unId : null,
            host: this.host ? this.host.unId : null,
        };
    }
}
