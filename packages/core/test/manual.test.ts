...
const validateTree = (node: Node = DynamixLayoutCore._root): boolean => {
	...
	for (const currentNode of allNodes) {
		...
		if (currentNode !== DynamixLayoutCore._root) {
			if (!currentNode.host) {
				...
				console.error(
					`❌ DISOWNED: ${id}'s host (uid: ${currentNode.host.unId}) does not contain it in its kids queue.`
				)
				nodeIsValid = false
			}
		}
		...
		for (const kid of currentNode.kids) {
			if (kid.host !== currentNode) {
				console.error(
					`❌ WRONG PARENT: Child (uid: ${kid.unId}) has host (uid: ${kid.host?.unId}), but should be ${currentNode.unId}.`
				)
				nodeIsValid = false
			}
		}
		...
		if (currentNode !== DynamixLayoutCore._root && currentNode.host) {
			...
			if (parentDir === undefined) {
				console.error(
					`❌ MISSING PARENT DIR: Host (uid: ${currentNode.host.unId}) of ${id} has no direction set in the cache.`
				)
				nodeIsValid = false
			} else if (currentDir === undefined) {
				console.error(
					`❌ MISSING OWN DIR: ${id} has no direction set in the cache.`
				)
				nodeIsValid = false
			} else if (currentDir === parentDir) {
				console.error(
					`❌ DIRECTION CONFLICT: ${id} has the same direction (${currentDir}) as its host. It should be the opposite.`
				)
				nodeIsValid = false
			}
		}
		...
		if (currentNode.type === 'row' || currentNode.type === 'tabset') {
			...
			if (kidsArray.length > 0) {
				if (kidsArray[0].prev !== null) {
					console.error(
						`❌ BROKEN START: First child (uid: ${kidsArray[0].unId})'s 'prev' link is not null.`
					)
					nodeIsValid = false
				}
				if (kidsArray[kidsArray.length - 1].next !== null) {
					console.error(
						`❌ BROKEN END: Last child (uid: ${kidsArray[kidsArray.length - 1].unId})'s 'next' link is not null.`
					)
					nodeIsValid = false
				}
				...
				for (let i = 0; i < kidsArray.length - 1; i++) {
					...
					if (!currentKid.next) {
						console.error(
							`❌ BROKEN CHAIN: Child (uid: ${currentKid.unId}) is missing its 'next' bond.`
						)
						nodeIsValid = false
						continue
					}
					if (currentKid.next.next !== nextKid) {
						console.error(
							`❌ FORWARD LINK: Child (uid: ${currentKid.unId})'s 'next' bond does not point to the correct next sibling (uid: ${nextKid.unId}).`
						)
						nodeIsValid = false
					}
					if (!nextKid.prev) {
						console.error(
							`❌ BROKEN CHAIN: Child (uid: ${nextKid.unId}) is missing its 'prev' bond.`
						)
						nodeIsValid = false
						continue
					}
					if (nextKid.prev.prev !== currentKid) {
						console.error(
							`❌ BACKWARD LINK: Child (uid: ${nextKid.unId})'s 'prev' bond does not point back to the correct previous sibling (uid: ${currentKid.unId}).`
						)
						nodeIsValid = false
					}
					if (currentKid.next.host !== currentNode) {
						console.error(
							`❌ BOND HOST: Bond between ${currentKid.unId} and ${nextKid.unId} has incorrect host (uid: ${currentKid.next.host?.unId}). Should be ${currentNode.unId}.`
						)
						nodeIsValid = false
					}
				}
	...
	if (isValid) {
		console.log(
			'✅ Validation Complete: Tree integrity and directions are OK.'
		)
	} else {
		console.error(
			'❌ Validation Complete: Found one or more issues in the tree structure.'
		)
	}
	console.groupEnd()
								...
								nodKids: [
									{
										...
										nodName: '',
										uidNode:
											'c0d0d6e5-c4bc-4146-8911-017257757d2a',
										nodOpen: 'editor',
										nodKids: [
											{
												typNode: 'tab',
									...
									},
									{
										...
										nodName: '',
										uidNode:
											'10fbe6c8-18d4-439a-ad90-21fd3241141c',
										nodKids: [
											{
												typNode: 'tabset',
												nodPart: 100,
